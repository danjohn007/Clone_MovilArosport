...
useEffect(() => {
  if (!rondaIds || !rondaIds.idRondaJuego || !rondaIds.idRondaJuego2) return;
  ...
  const intervalId = setInterval(async () => {
    try {
      ...
      const nuevoEstado = marcador1[4];
      setEstadoPartida(nuevoEstado);

      if (nuevoEstado === "2") {
        // Tie break enviado → bloqueamos edición, mostramos tie break
        setEditable(false);
        setHayTiebreak(true);
      } else if (nuevoEstado === "3") {
        // Ronda guardada → bloqueamos edición, puedes ocultar tie break si quieres
        setEditable(false);
        // Aquí decides si quieres mantener el tie break visible o no
        setHayTiebreak(!!marcador1[3]); 
      } else if (nuevoEstado === "1") {
        // Partida en juego → editable, no hay tie break visible
        setEditable(true);
        setHayTiebreak(false);
      }

      // Actualizar puntajes si cambian
      const updatedScores = {
      ...
      if (scoresChanged) {
        ...
        // Si el marcador es 7-6 o 6-7 y hay tie break confirmado, lo pasamos
        if (
      ...
      console.error("Error al verificar marcador:", error);
    }
  }, 200);
...
const handleTerminarRonda = async () => {
  console.log("🚀 handleTerminarRonda iniciado");
  console.log("Modo de juego:", modoJuego);
  console.log("Puntos actuales:", puntos1, puntos2);
  console.log("Rondas IDs:", rondaIds);
  ...
  if (modoJuego == "15" || modoJuego == 15 || modoJuego == "1" || modoJuego == 1) {
    ...
    console.log("Validación americana:", error);
    if (error) {
      setErrorMsg(error);
    ...
    console.log("Validación tiempo real:", error);
    if (error) {
      setErrorMsg(error);
      console.warn("Error de validación:", error);
      return;
...
const nuevasCerradas = { 
  ...rondasCerradas, 
  [rondaIds.idRondaJuego]: true,
  [rondaIds.idRondaJuego2]: true
};

  console.log("Rondas cerradas antes del tie-break:", nuevasCerradas);

  if (puntos1 === "6" && puntos2 === "6") {
    console.log("Empate 6-6, mostrando tie-break");
  ...
  try {
    console.log("Llamando a terminarPartidaRetaMultiple con IDs:", [
      rondaIds.idRondaJuego,
      rondaIds.idRondaJuego2,
    ]);
const resultado = await terminarPartidaRetaMultiple(
  rondaIds.idRondaJuego,   // 7331
  rondaIds.idRondaJuego2   // 7332
);




console.log("Resultado completo Axios:", resultado);
console.log("Status del backend:", resultado.data?.status);
console.log("Mensaje del backend:", resultado.data?.message);
...
if (resultado.data?.status) {
  ...
  console.warn("No se pudo terminar la partida:", resultado.data?.message);
}
  ...
  console.log("Actualizando rondas cerradas y finalizando guardado");
  setRondasCerradas(nuevasCerradas);
  ...
  onConfirm={(puntajes) => {
    ...
    // Determinar ganador y actualizar puntos principales
    ...
    if (!isNaN(tie1) && !isNaN(tie2)) {
      ...
      // ✅ Aquí llamamos al backend para terminar la partida automáticamente
      if (rondaIds?.idRondaJuego && rondaIds?.idRondaJuego2) {
        console.log(
          "Enviando tie-break confirmado al backend con IDs:",
          rondaIds.idRondaJuego,
          rondaIds.idRondaJuego2
        );
        terminarPartidaRetaMultiple(
          ...
          .then((res) => {
            console.log("Partida terminada con tie-break:", res);
            setEstadoPartida("3"); // Partida finalizada
            setHayTiebreak(true); // Mostrar tie-break visualmente
            setRondasCerradas((prev) => ({
              ...prev,
              [rondaIds.idRondaJuego]: true,
              [rondaIds.idRondaJuego2]: true,
            }));
          ...
          .catch((err) => {
            console.error(
              "Error al terminar partida con tie-break:",
              err
            );
          });
      }
    } else {
      // Si no hay tie-break válido, mantener los puntos
      setTiebreakConfirmado(null);
      onPuntosChange({
